-- Força a remoção de todas as versões das funções do chat

-- Remover TODAS as versões possíveis das funções
DROP FUNCTION IF EXISTS insert_chat_ev_message(UUID, TEXT, TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS insert_chat_ev_message(UUID, TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS insert_chat_ev_message(UUID, TEXT, TEXT, TEXT, TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS get_chat_ev_messages(INTEGER, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS get_chat_ev_messages(INTEGER) CASCADE;
DROP FUNCTION IF EXISTS get_chat_ev_messages() CASCADE;

-- Aguardar um pouco para garantir que as funções foram removidas
SELECT pg_sleep(1);

-- Agora criar as funções corretas
