...
const verificarContextoAuth = async () => {
  try {
    ...
    if (navbar) {
      ...
      const userElements = navbar.querySelectorAll('[class*="User"], [class*="Profile"], [class*="Avatar"]');
    ...
    if (loginElements.length > 0) {
      console.log('Primeiros elementos de auth:');
      loginElements.slice(0, 3).forEach((el, i) => {
        console.log(`${i + 1}.`, el.tagName, el.className || 'sem classe', el.textContent?.substring(0, 50));
      });
    }

    // 4. Verificar se há elementos de erro de autenticação
    console.log('\n4️⃣ VERIFICANDO ERROS DE AUTENTICAÇÃO');
    ...
    // 5. Verificar se há elementos de carregamento
    console.log('\n5️⃣ VERIFICANDO ELEMENTOS DE CARREGAMENTO');
    ...
    // 6. Verificar se há elementos específicos do ChatEV
    console.log('\n6️⃣ VERIFICANDO ELEMENTOS ESPECÍFICOS DO CHAT');
    ...
    // 7. Verificar se há elementos de formulário
    console.log('\n7️⃣ VERIFICANDO ELEMENTOS DE FORMULÁRIO');
    ...
    // 8. Verificar se há elementos de mensagem
    console.log('\n8️⃣ VERIFICANDO ELEMENTOS DE MENSAGEM');
    ...
    // 9. Verificar se há elementos de container
    console.log('\n9️⃣ VERIFICANDO ELEMENTOS DE CONTAINER');
