-- Script para modificar o schema da votação para permitir múltiplas escolhas
...
-- 2. Remover a constraint UNIQUE atual que permite apenas 1 voto por usuário
ALTER TABLE mascot_votes DROP CONSTRAINT IF EXISTS mascot_votes_user_id_key;

-- 3. Adicionar uma nova constraint que permite até 3 votos por usuário
-- Primeiro, vamos criar uma função para verificar se o usuário já votou 3 vezes
...
-- 4. Criar o trigger para aplicar a nova regra
...
-- 5. Atualizar a função de verificação de voto para retornar quantos votos o usuário já fez
...
-- 6. Atualizar a função de resultados para mostrar votos múltiplos
...
-- 7. Criar função para obter os votos de um usuário específico
...
-- 8. Atualizar as políticas RLS para permitir múltiplas inserções
...
-- 9. Verificar se as modificações foram aplicadas
SELECT 
