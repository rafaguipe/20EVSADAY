...
-- Índice composto para queries por usuário + data (streak, today EVs, history)
...
-- Índice para count rápido de hoje (usado no dashboard e milestone)
CREATE INDEX IF NOT EXISTS idx_evs_user_date
  ON evs (user_id, date_trunc('day', created_at DESC));
